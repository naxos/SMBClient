...
`SMBClient` is built on top of [libdsm](http://videolabs.github.io/libdsm), a low level SMB client library written in C. A copy of `libdsm` is embedded in this library to eliminate external dependencies. Many thanks to the cool guys of the dsm team @videolabs who must have gone to hell and back to produce their excellent code. Please refer to [COPYING.md](COPYING.md) to learn about licensing `SMBClient` including `libdsm`.
